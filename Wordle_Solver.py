import random
import string
from big_list import valid_WORDS, frequency_WORDS    # cached list of all valid words and dict of frequency of words
...
def letters_used():
    ...
    print(f"Grayed out letters (don't guess words with these {len(sorted_set)} letters): {''.join(sorted_set)}")

if __name__ == "__main__":
 
    print("Wordle Solver\n")

    print('Random first guess:', colored(f'{random_guess(valid_WORDS).upper()}', 'red'))
    previous_possibilities = valid_WORDS    # initialize to whole valid word list
    len_of_possibilities = [len(valid_WORDS)]   # initialize to length of whole valid word list
    ...
    while len_of_possibilities[-1] != 1:    # loop until only one guess is left
        ...
        # get user input
        print("Enter " + colored("green ", "green") + "letters (return/enter for empty)")
        ...
        print("Enter " + colored("yellow ", "yellow") + "letters (return/enter for empty)")
        ...
        print("Enter " + colored("gray ", "grey") + "letters (return/enter for empty)")
        ...
        if (len(possibilities)) <= 6 and (len(possibilities) != 1):
            ...
            for word in possibilities:
                if word == next_guess:
                    ...
                    print(word.upper())
        previous_possibilities = possibilities  # set next rounds word list to this rounds solutions
